...
public class OrganizationEventServiceImplementation implements IEventService<Organization> {
    ...
    private static final Log log = LogFactory.getLog(OrganizationEventServiceImplementation.class);
    ...
    @Override
    public void publish(EventType type, Organization entity) {
        try {
    ...
    @Override
    public CompletableFuture<NotificationMessage> getNotificationMessage(EventType type, Organization entity) {
        String stringNotificationMessage = "";
        switch(type) {
            case CREATED:
                stringNotificationMessage = String.format("Your Organization %s has been created successfully", entity.getName());
                break;
            case UPDATED:
                stringNotificationMessage = String.format("The Organization named %s has been updated successfully", entity.getName());
                break;
            case DELETED:
                stringNotificationMessage = String.format("The Organization named %s has been deleted successfully", entity.getName());
                break;
            default:
                // do nothing
        }
